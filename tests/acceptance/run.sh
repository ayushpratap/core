...
remote_occ() {
	CURL_OCC_RESULT=`curl -s -u admin:$1 $2 -d "command=$3"`
...
then
	remote_occ $ADMIN_PASSWORD $OCC_URL "config:system:delete sharing.federation.allowHttpFallback"
else
	remote_occ $ADMIN_PASSWORD $OCC_URL "config:system:set sharing.federation.allowHttpFallback --type boolean --value=$PREVIOUS_HTTP_FALLBACK_SETTING"
fi
